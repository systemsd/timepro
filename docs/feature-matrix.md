...
| **Installer downloads + auto-update** | ✅ | Download page resolves the **latest Release of the separate PUBLIC `timepro-downloads` repo**; CI (`desktop-release.yml`, mac + Windows) publishes there via the `RELEASES_REPO_TOKEN` PAT on each version bump. **In-app auto-updater** live (v0.1.5+; polls `latest.json`). Current shipped **v0.1.11**. Still **unsigned** (each mac update revokes Screen Recording — Phase 7 sign/notarize pending); Linux leg disabled. |
