...
| **B6 — Settings engine** | Settings page (S11), Team per-user overrides (S7), agent behavior | ✅ |
...
| **B9 — Build/sign/host pipeline** | Download page artifacts (S12) | 🟡 (backend deploy pipeline in progress — see [docs/14](14-deploy-and-download-progress.md); installers not built) |
...
>
> **C7 now resolved too** (My Account vs Settings — see below): two surfaces, `/account` (per-user) + `/settings` (org). No open conflicts remain.
...
### C7 — "My Account" vs "Settings" — ✅ RESOLVED
- **Ruling: two distinct surfaces.** **My Account** (`/account`, per-user) = the signed-in person's own
  profile + security + personal API token, reached from the **avatar dropdown** (Dashboard · My Account · Log out).
  **Settings** (`/settings`, org-scoped, admin-only) = org policy defaults + per-user overrides via the ☰ menu.
- **Built:** avatar hover-dropdown in `TopNav.tsx`; `apps/web/src/app/account/page.tsx` (profile from `GET /v1/me/profile`
  — name/org/email/role + viewer tz). Profile/security actions (edit, change password/email, 2FA, delete, API token)
  are **disabled placeholders** until Phase 6 real auth backs them.
